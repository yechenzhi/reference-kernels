...
__global__ void gray_kernel(const scalar_t* __restrict__ data,
    ...
    __shared__ scalar_t data_s[32][32][3];
    __shared__ scalar_t output_s[32][32];

    for(int i = 0; i < 3; ++i) {
        data_s[threadIdx.y][threadIdx.x][i] = data[row * N * 3 + col * 3 + i];
    }
    __syncthreads();
    ...
    if (row < N && col < N) {
        scalar_t sum = 0.0f;
        for (int i = 0; i < 3; ++i) {
            sum += data_s[threadIdx.y][threadIdx.x][i] * weights[i];
        }
        output_s[threadIdx.y][threadIdx.x] = sum;   
    }
    __syncthreads();
    if (row < N && col < N) {
        C[row * N + col] = output_s[threadIdx.y][threadIdx.x];
    }
}
